...
extension HealthDataStore {
  ...
  func runIMUStepCount() {
    let db = databasePath
    let bridge = self.bridge
    let now = Date().timeIntervalSince1970
    let windowStart = now - 24 * 3600
    let deviceID = "goose.swift.imu.steps.v1"
    // Capture WHOOP step count from packetInputReports on @MainActor.
    ...
    packetInputQueue.async { [weak self] in
      ...
      do {
        gravityReport = try bridge.request(
      ...
      Task { @MainActor [weak self] in
        self?.imuStepCountResult = imuResult
      }
    }
  }
}
